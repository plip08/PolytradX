...
export class AiAgentConnector implements Strategy {
  ...
  async evaluate(context: StrategyContext): Promise<StrategyDecision | null> {
    logger.debug("AiAgentConnector evaluate", { marketId: context.currentState.marketId });
    return null;
  }

  async execute(_decision: StrategyDecision): Promise<StrategyResult> {
    logger.info("AiAgentConnector execute placeholder");
    return { success: true, message: "AiAgentConnector placeholder executed." };
  }
}
